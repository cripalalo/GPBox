...
class eBroker(object):
	def proceso(self):
		...
		precioMax = dato.precio[0]+0.35


		#--- RED TRAMO 1 ---#
		#potR1 = dato.x[18]
		potR1 = dato.x[18]*float(potCont)
		preRed1D = 0
		prioRed1O = 5
		prioRed1D = 5

		#--- RED TRAMO 2 ---#
		potR2 = (dato.x[19]-dato.x[18])*float(potCont) 
		preRed2D = 0
		prioRed2O = 5
		prioRed2D = 5
		...
		#--- Precios de las cargas ---#
		precioFija = np.ones((1,3))
		precioOnOff = np.array((dato.x[0],dato.x[1],dato.x[2]))
		precioGest = np.array((dato.x[3],dato.x[4],dato.x[5]))
		precioGest1 = np.array((dato.x[3]-dato.x[6],dato.x[4]-dato.x[7],dato.x[5]-dato.x[8]))
		precioGest2 = np.array((dato.x[3]-dato.x[9],dato.x[4]-dato.x[10],dato.x[5]-dato.x[11]))
		precioGest3 = np.array((dato.x[3]-dato.x[12],dato.x[4]-dato.x[13],dato.x[5]-dato.x[14]))
		precioGest4 = np.array((dato.x[3]-dato.x[15],dato.x[4]-dato.x[16],dato.x[5]-dato.x[17]))
		...
		for k in range(0,10,1):
			...
			dato.MD[k][0] = prioLoad
			dato.MD[k][3] = prioLoad
		...
		dato.MO[0][2] = float(dato.precio[i-1])/precioMax
		dato.MD[0][2] = 0
		...
		dato.MO[1][2] = (float(dato.precio[i-1])+0.1)/precioMax
		dato.MD[1][2] = 0
		...
		dato.MO[2][2] = (float(dato.precio[i-1]) + 0.35)/precioMax
		dato.MD[2][2] = 0
		...
		if dato.intervalo == 2976:
			...
			for j in range(0,3,1):
				if resultado.picoCarga[j] < 0.85*definiciones.potCont:
					...
					resultado.costePotencia = resultado.costePotencia+definiciones.tp1[j]*(resultado.picoCarga[j]+2*resultado.picoCarga[j])
				else:
